...
- `bun cmd/bench-sum.ts [file.djvu] [-clang] [-full]` — same harness as
  ...
  **SumatraPDF** actually opens/renders pages instead of timing the bare
  ...
  decoder cost). Historically `bench.ts` showed us faster while libdjvu won in
  SumatraPDF because our engine ran an extra full-frame pass libdjvu avoids:
  libdjvu renders straight into the caller's buffer in BGR (its native pixel
